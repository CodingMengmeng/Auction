...
import com.example.auctionapp.wxapi.domain.AccountFans;
import com.example.auctionapp.wxapi.domain.MsgNews;
import com.example.auctionapp.wxapi.vo.Material;
import com.example.auctionapp.wxapi.vo.MaterialArticle;
import com.example.auctionapp.wxapi.vo.MaterialItem;
...
public class WxApiClient {
	 ...
	 * @return
	 */
	public  List<MpAccount> getAllMpAccountList() {
		return wxMemoryCacheClient.getAllMpAccountMaps();
	}

	// 获取accessToken
	public  String getAccessToken(MpAccount mpAccount) {
		...
		if (token != null && !token.isExpires()) {// 不为空，并且没有过期
			...
			if (token != null) {
				if (token.getErrcode() != null) {// 获取失败
					log.error("## getAccessToken Errcode[{}] ErrorMsg [{}]", token.getErrcode(), token.getErrmsg());
				} else {
	...
	// 刷新AccessToken
	public  void doRefreshAccessToken(MpAccount mpAccount) {
		// 获取唯一的accessToken，如果是多账号，请自行处理
		AccessToken token = WxApi.getAccessToken(mpAccount.getAppid(), mpAccount.getAppsecret());
		if (token != null) {
			if (token.getErrcode() != null) {// 获取失败
				log.error("## getAccessToken Error [{}]", JSONObject.fromObject(token).toString());
			} else {
				wxMemoryCacheClient.addAccessToken(mpAccount.getAccount(), token);
			}
		}
	...
	public  String getOAuthOpenId(MpAccount mpAccount, String code) {
		...
		if (msg == null || msg.getErrcode() != 0) {
			log.error("## getValidAccessToken Error [{}]", msg.getErrmsg());
		}
		if (token != null) {
			// 获取失败
	...
	public  AccountFans syncAccountFans(String openId, MpAccount mpAccount) {
		...
		if (null != jsonObj) {
			if (jsonObj.containsKey("errcode")) {
				...
				log.error(String.format("获取用户信息失败 errcode:{} errmsg:{}", errorCode, ErrCode.errMsg(errorCode)));
				return null;
			} else {
	...
	public  Material syncBatchMaterial(MediaType mediaType, Integer offset, Integer count, MpAccount mpAccount) {
		...
		try {
			...
			if (jsonObj.containsKey("errcode")) {// 获取素材失败
				log.error(ErrCode.errMsg(jsonObj.getInt("errcode")));
				return null;
			} else {
	 ...
	 * @return
	 */
	public  JSONObject sendCustomerNewsMessage(String openid, List<MsgNews> msgNews, MpAccount mpAccount) {
	 ...
	 * @return
	 */
	public  JSONObject updateMembersGorups(String openid, String togroupid, MpAccount mpAccount) {
		JSONObject postObj = new JSONObject();
	 ...
	 * @return
	 */
	public  JSONObject getLongToShort(String longUrl, MpAccount mpAccount) {
		String accessToken = getAccessToken(mpAccount);
